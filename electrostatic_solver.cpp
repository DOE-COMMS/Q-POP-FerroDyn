...
#pragma acc loop gang vector private(mat_type,mat,er_local,x,y,z)
	for (long i = 0; i < n; i++) {
		mat_type = pt_glb->material_cell(i);
		if (mat_type != 0) {
			mat = &(pt_glb->material_parameters[(mat_type)-1]);
			er_local = mat->r_permittivity;
		}
		else {
			er_local = 1.;
		}
		...
		if (pt_glb->if_elec_1D_compensate == false) {
			...
			Ex_stat(i) = 0.;
			Ey_stat(i) = 0.;
			Ez_stat(i) = (0.5 * pt_glb->free_charge_cells[z] - pz_glb_store(i)) / e0 / er_local;
		}
		else {
			Ex_stat(i) = 0.;
			Ey_stat(i) = 0.;
