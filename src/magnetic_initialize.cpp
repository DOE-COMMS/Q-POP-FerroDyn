			...
			for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
				...
				for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
					...
					if (pt_geo->periodicX == true) {
						if (if_F1 ^ if_F2) {
							FM_surfYZ(pt_geo->xS, j, k) = true;
							FM_surfYZ(pt_geo->xE, j, k) = true;
						}
						else {
							FM_surfYZ(pt_geo->xS, j, k) = false;
							FM_surfYZ(pt_geo->xE, j, k) = false;
						}
					}
			...
			for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
				...
				for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
					...
					if (pt_geo->periodicY == true) {
						if (if_F1 ^ if_F2) {
							FM_surfXZ(i, pt_geo->yS, k) = true;
							FM_surfXZ(i, pt_geo->yE, k) = true;
						}
						else {
							FM_surfXZ(i, pt_geo->yS, k) = false;
							FM_surfXZ(i, pt_geo->yE, k) = false;
						}
					}
			...
			for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
				...
				for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
					...
					if (pt_geo->periodicZ == true) {
						if (if_F1 ^ if_F2) {
							FM_surfXY(i, j, pt_geo->zS) = true;
							FM_surfXY(i, j, pt_geo->zE) = true;
						}
						else {
							FM_surfXY(i, j, pt_geo->zS) = false;
							FM_surfXY(i, j, pt_geo->zE) = false;
						}
					}
			...
			if (mat_type != 0) {
				...
				if (mat->if_FM == true) {
					...
					if (FM_surfYZ(x + 1, y, z) == true) {
						...
						if (x == pt_geo->xE - 1) {
							mat_typen = pt_glb->material_cell(pt_geo->xS, y, z);
					...
					if (FM_surfYZ(x, y, z) == true) {
						...
						if (x == pt_geo->xS) {
							mat_typen = pt_glb->material_cell(pt_geo->xE - 1, y, z);
							matn = &(pt_glb->material_parameters[(mat_typen)-1]);
							Aex1 = mat->Aex; Aex2 = matn->Aex;
							//Ms1 = mat->Ms; Ms2 = matn->Ms;
					...
					if (FM_surfXZ(x, y + 1, z) == true) {
						...
						if (y == pt_geo->yE - 1) {
							mat_typen = pt_glb->material_cell(x, pt_geo->yS, z);
							matn = &(pt_glb->material_parameters[(mat_typen)-1]);
							Aex1 = mat->Aex; Aex2 = matn->Aex;
							//Ms1 = mat->Ms; Ms2 = matn->Ms;
					...
					if (FM_surfXZ(x, y, z) == true) {
						...
						if (y == pt_geo->yS) {
							mat_typen = pt_glb->material_cell(x, pt_geo->yE - 1, z);
							matn = &(pt_glb->material_parameters[(mat_typen)-1]);
							Aex1 = mat->Aex; Aex2 = matn->Aex;
							//Ms1 = mat->Ms; Ms2 = matn->Ms;
					...
					if (FM_surfXY(x, y, z + 1) == true) {
						...
						if (z == pt_geo->zE - 1) {
							mat_typen = pt_glb->material_cell(x, y, pt_geo->zS);
							matn = &(pt_glb->material_parameters[(mat_typen)-1]);
							Aex1 = mat->Aex; Aex2 = matn->Aex;
							//Ms1 = mat->Ms; Ms2 = matn->Ms;
					...
					if (FM_surfXY(x, y, z) == true) {
						...
						if (z == pt_geo->zS) {
							mat_typen = pt_glb->material_cell(x, y, pt_geo->zE - 1);
							matn = &(pt_glb->material_parameters[(mat_typen)-1]);
							Aex1 = mat->Aex; Aex2 = matn->Aex;
							//Ms1 = mat->Ms; Ms2 = matn->Ms;
	...
	if (pt_glb->if_AFM_all == true) {
		{
			...
			for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
				for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
					...
					if (pt_geo->periodicX == true) {
						if (if_F1 ^ if_F2) {
							AFM_surfYZ(pt_geo->xS, j, k) = true;
							AFM_surfYZ(pt_geo->xE, j, k) = true;
						}
						else {
							AFM_surfYZ(pt_geo->xS, j, k) = false;
							AFM_surfYZ(pt_geo->xE, j, k) = false;
						}
					}
					else if (pt_geo->periodicX == false) {
						if (if_F1 == true) {
			...
			for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
				for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
					...
					if (pt_geo->periodicY == true) {
						if (if_F1 ^ if_F2) {
							AFM_surfXZ(i, pt_geo->yS, k) = true;
							AFM_surfXZ(i, pt_geo->yE, k) = true;
						}
						else {
							AFM_surfXZ(i, pt_geo->yS, k) = false;
							AFM_surfXZ(i, pt_geo->yE, k) = false;
						}
					}
					else if (pt_geo->periodicY == false) {
						if (if_F1 == true) {
			...
			for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
				for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
					...
					if (pt_geo->periodicZ == true) {
						if (if_F1 ^ if_F2) {
							AFM_surfXY(i, j, pt_geo->zS) = true;
							AFM_surfXY(i, j, pt_geo->zE) = true;
						}
						else {
							AFM_surfXY(i, j, pt_geo->zS) = false;
							AFM_surfXY(i, j, pt_geo->zE) = false;
						}
					}
					else if (pt_geo->periodicZ == false) {
						if (if_F1 == true) {
