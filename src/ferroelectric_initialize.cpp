...
void ferroelectric_system::initialize_host() {
	...
	if (pt_glb->if_FE_all == true) {
		...
		for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
			for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
				material_type1 = pt_glb->material_cell(pt_geo->xS, j, k);
				if (material_type1 == 0) {
					if_F1 = false;
				}
				else {
				...
				material_type2 = pt_glb->material_cell(pt_geo->xE - 1, j, k);
				if (material_type2 == 0) {
					if_F2 = false;
				}
				else {
				...
				if (pt_geo->periodicX == true) {
					if (if_F1 ^ if_F2) {
						FE_surfYZ(pt_geo->xS, j, k) = true;
						FE_surfYZ(pt_geo->xE, j, k) = true;
					}
					else {
						FE_surfYZ(pt_geo->xS, j, k) = false;
						FE_surfYZ(pt_geo->xE, j, k) = false;
					}
				}
		...
		for (long int i = pt_geo->xS + 1; i < pt_geo->xE; i++) {
			for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
				for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
		...
		for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
			for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
				material_type1 = pt_glb->material_cell(i, pt_geo->yS, k);
				if (material_type1 == 0) {
					if_F1 = false;
				}
				else {
				...
				material_type2 = pt_glb->material_cell(i, pt_geo->yE - 1, k);
				if (material_type2 == 0) {
					if_F2 = false;
				}
				else {
				...
				if (pt_geo->periodicY == true) {
					if (if_F1 ^ if_F2) {
						FE_surfXZ(i, pt_geo->yS, k) = true;
						FE_surfXZ(i, pt_geo->yE, k) = true;
					}
					else {
						FE_surfXZ(i, pt_geo->yS, k) = false;
						FE_surfXZ(i, pt_geo->yE, k) = false;
					}
				}
		...
		for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
			for (long int j = pt_geo->yS + 1; j < pt_geo->yE; j++) {
				for (long int k = pt_geo->zS; k < pt_geo->zE; k++) {
		...
		for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
			for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
				material_type1 = pt_glb->material_cell(i, j, pt_geo->zS);
				if (material_type1 == 0) {
					if_F1 = false;
				}
				else {
				...
				material_type2 = pt_glb->material_cell(i, j, pt_geo->zE - 1);
				if (material_type2 == 0) {
					if_F2 = false;
				}
				else {
				...
				if (pt_geo->periodicZ == true) {
					if (if_F1 ^ if_F2) {
						FE_surfXY(i, j, pt_geo->zS) = true;
						FE_surfXY(i, j, pt_geo->zE) = true;
					}
					else {
						FE_surfXY(i, j, pt_geo->zS) = false;
						FE_surfXY(i, j, pt_geo->zE) = false;
					}
				}
		...
		for (long int i = pt_geo->xS; i < pt_geo->xE; i++) {
			for (long int j = pt_geo->yS; j < pt_geo->yE; j++) {
				for (long int k = pt_geo->zS + 1; k < pt_geo->zE; k++) {
	...
	er_homo11 = pt_glb->material_parameters[0].r_permittivity11;
	er_homo12 = pt_glb->material_parameters[0].r_permittivity12;
	er_homo13 = pt_glb->material_parameters[0].r_permittivity13;

	er_homo21 = pt_glb->material_parameters[0].r_permittivity21;
	er_homo22 = pt_glb->material_parameters[0].r_permittivity22;
	er_homo23 = pt_glb->material_parameters[0].r_permittivity23;

	er_homo31 = pt_glb->material_parameters[0].r_permittivity31;
	er_homo32 = pt_glb->material_parameters[0].r_permittivity32;
	er_homo33 = pt_glb->material_parameters[0].r_permittivity33;

	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity11 > er_homo11) {
			er_homo11 = pt_glb->material_parameters[i].r_permittivity11;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity12 > er_homo12) {
			er_homo12 = pt_glb->material_parameters[i].r_permittivity12;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity13 > er_homo13) {
			er_homo13 = pt_glb->material_parameters[i].r_permittivity13;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity21 > er_homo21) {
			er_homo21 = pt_glb->material_parameters[i].r_permittivity21;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity22 > er_homo22) {
			er_homo22 = pt_glb->material_parameters[i].r_permittivity22;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity23 > er_homo23) {
			er_homo23 = pt_glb->material_parameters[i].r_permittivity23;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity31 > er_homo31) {
			er_homo31 = pt_glb->material_parameters[i].r_permittivity31;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity32 > er_homo32) {
			er_homo32 = pt_glb->material_parameters[i].r_permittivity32;
		}
	}
	for (long int i = 1; i < pt_glb->num_materials; i++) {
		if (pt_glb->material_parameters[i].r_permittivity33 > er_homo33) {
			er_homo33 = pt_glb->material_parameters[i].r_permittivity33;
		}
	}
	er_homo11 = er_homo11 * pt_glb->scale_elec;
	er_homo12 = er_homo12 * pt_glb->scale_elec;
	er_homo13 = er_homo13 * pt_glb->scale_elec;
	er_homo21 = er_homo21 * pt_glb->scale_elec;
	er_homo22 = er_homo22 * pt_glb->scale_elec;
	er_homo23 = er_homo23 * pt_glb->scale_elec;
	er_homo31 = er_homo31 * pt_glb->scale_elec;
	er_homo32 = er_homo32 * pt_glb->scale_elec;
	er_homo33 = er_homo33 * pt_glb->scale_elec;
	...
	for (long int i = 0; i < nx; i++) {
		for (long int j = 0; j < ny; j++) {
			for (long int k = 0; k < nz / 2 + 1; k++) {
				temporary_var = er_homo33 * pt_glb->k_norm(i, j, k);
